...
def profitability_scenarios(model: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    defaults = {str(metric["id"]): float(metric["default"]) for metric in data.get("metrics", [])}
    required = ["monthly_active_players", "payer_conversion", "arppu", "hosting_cost", "tooling_cost", "asset_budget"]
    missing = [item for item in required if item not in defaults]
    if missing:
        raise ValueError("profitability model missing metrics: " + ", ".join(missing))

    fixed_cost = defaults["hosting_cost"] + defaults["tooling_cost"] + defaults["asset_budget"]
    rows: list[dict[str, Any]] = []
    for scenario in data.get("scenarios", []):
        players = defaults["monthly_active_players"] * float(scenario.get("active_players_multiplier", 1))
        conversion = defaults["payer_conversion"] * float(scenario.get("conversion_multiplier", 1)) / 100
        arppu = defaults["arppu"] * float(scenario.get("arppu_multiplier", 1))
...
def validate_intelligence_configs(root: Path | None = None) -> dict[str, Any]:
    ...
    scenarios = profitability_scenarios(profit)
    if len(scenarios) < 3:
        raise ValueError("profitability model requires at least 3 scenarios")
...
def intelligence_status() -> dict[str, Any]:
    ...
    return {
        ...
        "profitability": {
            ...
            "scenarios": validation["profitability_scenarios"],
